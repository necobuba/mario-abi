...
function endCastleInsideFinal(xloc, last) {
  ...
  if(last) {
    ...
    text = stopper.text = [
      pushPreText({innerHTML: "THX MARIO!", style: style}, xloc + 160, 66).object,
      pushPreText({innerHTML: "YOUR QUEST IS OVER.<BR>SIZE YENI BIR GÖREV SUNUYORUZ.", style: style}, xloc + 148, 50).object,
      pushPreText({innerHTML: "B TUSUNA BASIN<BR>BIR DUNYA SECMEK ICIN", style: style}, xloc + 148, 26).object
    ];
  }
  // ...or that jerk Toad
  else {
...
function World11(map) {
  ...
  map.areas = [
    new Area("Overworld", function() {
      ...
      greeter += "    <span style='font-size:84px'>mario</span>";
      greeter += "    <br><br>"; // To do: make this not so font dependant
      greeter += "    <span style='font-size:75px;line-height:76px'>ABI HTML5</span>";
      greeter += "  </p>";
      greeter += "</div>";
      greeter += "<p id='explanation' style='text-align:center;<!--/*text-shadow:2px 2px 1px black;*/-->margin-left:7px;'>";
      greeter += "  Arrow/WASD tuslari ile hareket et";
      greeter += "  <br>";
      greeter += "  Shift e basarak/ kosabilirsin";
      greeter += "  <br>";
      greeter += "  P/M to DURDUR/SUSTUR";
      // greeter += "  <br>";
